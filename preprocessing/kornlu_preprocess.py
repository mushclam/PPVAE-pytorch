...
def main(args):
    train_data = open_tsv(args.train_path)
    valid_data = open_tsv(args.valid_path)
    test_data = open_tsv(args.test_path)

    train_data, valid_data, test_data = \
        map(flatten, [train_data, valid_data, test_data])

    mecab = Mecab()
    train_data = morph_list(train_data, mecab, desc='Train')
    valid_data = morph_list(valid_data, mecab, desc='Valid')
    test_data = morph_list(test_data, mecab, desc='Test')

    output_path = Path(args.output_dir)
    if not output_path.exists():
        output_path.mkdir()

    save_pickle(train_data, os.path.join(args.output_dir, 'kornli_train.pkl'))
    save_pickle(valid_data, os.path.join(args.output_dir, 'kornli_valid.pkl'))
    save_pickle(test_data, os.path.join(args.output_dir, 'kornli_test.pkl'))
